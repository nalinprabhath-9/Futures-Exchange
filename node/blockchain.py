...
from node.crypto_utils import sign_message, get_signing_key_from_hex, get_compressed_pubkey, verify_signature
...
class BalanceManager:
    ...
    def lock_collateral_party(self, trade_id: str, party: str, amount: int, role: str) -> bool:
        ...
        if self.get_available_balance(party) < amount:
            print(
                f"Insufficient balance for {party[:20]}... (need {amount / MILLI_DENOMINATION} {CRYPTOCURRENCY_NAME})")
            return False
...
class Blockchain:
            ...
            if fee > 0:
                ...
                if fee_payer:
                    if self.balances.get_available_balance(fee_payer) >= fee:
                        ...
                        print(
                            f"  Collected fee: {fee / MILLI_DENOMINATION} {CRYPTOCURRENCY_NAME} from {fee_payer[:20]}...")
                    else:
                        print(
    ...
    def _expire_stale_proposals(self, current_time: int):
        ...
        for trade_id in expired:
            ...
            print(
                f"Trade {trade_id} EXPIRED (no acceptance by {datetime.fromtimestamp(trade.expiry_timestamp)}), collateral returned")
...
class TxnMemoryPool:
    ...
    def get_transactions(self, count: int) -> List[Transaction]:
        ...
        if result:
            ...
            print(
                f"Mempool: Selected {len(result)} transactions ({high_count} high-priority, {len(result) - high_count} normal)")
        return result
...
class Miner:
    ...
    def mine_block(self, blockchain: Blockchain, mempool: TxnMemoryPool, verbose: bool = True) -> Block:
        ...
        if verbose:
            print(
                f"Created coinbase: {COINBASE_REWARD / MILLI_DENOMINATION} {CRYPTOCURRENCY_NAME} -> {self.miner_address[:20]}...")
